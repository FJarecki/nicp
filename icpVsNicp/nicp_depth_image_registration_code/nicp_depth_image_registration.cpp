...
int main(int argc, char **argv) {
  ...
  cameraMatrix <<
    1078.68499f,   0.0f, 952.6592286f,
      0.0f, 1076.4742562f, 530.7386644f,
      0.0f,   0.0f,   1.0f;

  // Create the PinholePointProjector
  nicp::PinholePointProjector pointProjector;
  pointProjector.setMinDistance(0.85f);
  pointProjector.setMaxDistance(3.5f);
  pointProjector.setCameraMatrix(cameraMatrix);
  pointProjector.setImageSize(1080, 1920);
  ...
  outfile.open("nicp_transitions.txt", std::ios_base::app);//std::ios_base::app
  outfile << T.matrix()(0, 0) << " " << T.matrix()(0, 1)<< " " << T.matrix()(0, 2) << " " 
  << T.matrix()(1, 0) << " " << T.matrix()(1, 1) << " " << T.matrix()(1, 2) << " " 
  << T.matrix()(2, 0) << " " << T.matrix()(2, 1) << " " << T.matrix()(2, 2) << " " 
  << T.matrix()(0, 3) << " " << T.matrix()(1, 3) << " " << T.matrix()(2, 3) << "\n" ;
  outfile.close();
